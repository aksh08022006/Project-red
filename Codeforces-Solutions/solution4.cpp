#include <bits/stdc++.h>
    ...
    void solve(ll tc = 0) {
        cin >> n >> x;        vector<ll> a(n);                for (ll i = 0; i < n; i++) {            cin >> a[i];        }        sort(a.rbegin(), a.rend());                 ll grp = 0, mem = 0, mn = 1e9+7;                for (ll b : a) {            mem++;            mn = min(mn, b);            if (mem*mn>=x) {                grp++;                mem = 0;                mn = 1e9;             }        }        cout << grp << endl;
